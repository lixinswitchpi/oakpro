...
static int wctdm_init_voicedaa(struct wctdm *wc, int card, int fast, int manual, int sane)
{
	...
	wctdm_setreg(wc, card, 34, (card * 8) + 1);
	wctdm_setreg(wc, card, 35, 0x00);
	wctdm_setreg(wc, card, 36, (card * 8) + 1);
...
static int wctdm_init_proslic(struct wctdm *wc, int card, int fast, int manual, int sane)
{
    ...
    wctdm_setreg(wc, card, 2, (card * 8) + 1);    // Tx Start count low byte  0
   	//wctdm_setreg(wc, card, 2, (3-card) * 8);    // Tx Start count low byte  0
    wctdm_setreg(wc, card, 3, 0);    // Tx Start count high byte 0
    wctdm_setreg(wc, card, 4, (card * 8) + 1);    // Rx Start count low byte  0
